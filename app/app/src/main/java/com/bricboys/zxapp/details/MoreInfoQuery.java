...
public class MoreInfoQuery {
    ...
    public static MoreInfo extractInfo(String parametro) {
        ...
        try {
            ...
            if(_source.optJSONArray("originalprice") != null) {
                JSONArray price = _source.getJSONArray("originalprice");
                for (int i = 0; i < price.length(); i++) {
                    JSONObject obj = price.getJSONObject(i);
                    if (i == 0) {
                        mOriginalPrice = mOriginalPrice + obj.optString("currency") + " " +
            ...
            if(_source.optJSONArray("additionals") != null) {
                JSONArray addit = _source.getJSONArray("additionals");
                for (int i = 0; i < addit.length(); i++) {
                    JSONObject obj = addit.getJSONObject(i);
                    String type;
                    type = obj.optString("type");
                    if (type.equalsIgnoreCase("Advertisement")) {
                        mAdvertisement = obj.optString("url");
                        break;
                    }
                }
            }
